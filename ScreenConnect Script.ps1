...
function DownloadInstaller {
    ...
    if ( ($SupportedTLSversions -contains 'Tls13') -and ($SupportedTLSversions -contains 'Tls12') ) {
        ...
        Write-host "TLS 1.2 or TLS 1.3 isn't supported on this system. This download may fail!"
    ...
    try {
        ...
        $File = Test-Path -Path $InstallerFile -ErrorAction SilentlyContinue
    }
    catch {
        # File download failed, let's capture the error
        write-host "  Download failed"
        write-host "  $_.Exception.Message"
        if ($SupportedTLSversions -contains 'Tls13') {
            # Not everyone supports 1.3 but say they do, forcing 1.2 and trying again
            write-host "  Retrying with TLS1.2"
            try {
                [System.Net.ServicePointManager]::SecurityProtocol = [System.Net.SecurityProtocolType]::Tls12
                Invoke-WebRequest -Uri $DownloadURL -OutFile $InstallerFile
                $TestFile = Test-Path -Path $InstallerFile -ErrorAction SilentlyContinue
            }
            catch {
                write-host "  Second attempt failed, exiting script"
                exit 1
            }
        } else {
...
try {
    $TestWrite = New-Item -Path "test.txt" -ItemType File -ErrorAction SilentlyContinue
...
switch ($env:ScriptAction) {
    ...
    default {
        write-host "  No action called, exiting script"
    }
}
